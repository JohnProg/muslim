...
import { AnimationCurve, HorizontalAlignment, Visibility } from "ui/enums";
import { GestureTypes } from "ui/gestures";
import { Image } from "ui/image";
import { Label } from "ui/label";
import { GridLayout, GridUnitType, ItemSpec } from "ui/layouts/grid-layout";
import { StackLayout } from "ui/layouts/stack-layout";
import { getSlideshowImagesProfile } from "./api";

@Directive({selector: "[carousel]"})
export class CarouselDirective implements AfterViewInit {
    // Options
    @Input() carousel: any;
    @Input() carouselSpeed: number; // autoplay speed (ms) 播放速度
    @Input() carouselArrows: string; // arrows type  箭头类型
    @Input() carouselLabelOverlay: boolean; // title over image (bool)  标题在图像
    @Input() carouselAnimationSpeed: number; // animation speed 动画速度

    private animationSpeedDefault: number = 400; // in ms
    private autoPlaySpeedDefault: number = 0; // in ms
    ...
    private arrowType: number = CarouselArrowTypes.NONE;
     ...
     */
    private initOptions() {
        // Animation duration (in ms)  动画时间
        if (this.carouselAnimationSpeed && this.isNumeric(this.carouselAnimationSpeed)) {
            this.carouselAnimationSpeed = +this.carouselAnimationSpeed;
        } else {
            this.carouselAnimationSpeed = this.animationSpeedDefault;
        }

        // Autoplay (in ms) + animation duration  播放 + 动画时间
        if (this.carouselSpeed && this.isNumeric(this.carouselSpeed)) {
            this.carouselSpeed = +(this.carouselSpeed);
        } else {
            this.carouselSpeed = this.autoPlaySpeedDefault;
        }

        // Set arrow type  设置箭头类型
    ...
    /**
     * Init sliders layout  初始化滑块布局
     */
    private initImagesLayout() {
        this.totalItems = this.carousel.length;  // .ts 中 images（array） 的长度
     ...
     * 在“旋转木马”指令属性中提供的初始旋转木马滑块
     */
    private initSlides() {
        this.apollo.watchQuery<any>({
            query: getSlideshowImagesProfile,
            variables: {
                groupId: 1
            }
        }).valueChanges.subscribe(({ data }) => {
            const datas = data.getSlideshowImages;
            const code = datas.code;
            if (code === 200) {
                this.carousel = datas.images;
                this.totalItems = datas.images.length;
                this.carousel.forEach((slide: CarouselSlide, i: number) => {
                    ...
                    if (slide.url) {
                        const image: Image = this.generateImageSliderFromUrl(slide.url);
                        gridLayout.addChild(image);
                    }

                    if (slide.file && slide.file.indexOf("res://") !== 0) {
                        const image: Image = this.generateImageSliderFromFile(slide.file);
                        gridLayout.addChild(image);
                    }

                    if (slide.file && slide.file.indexOf("res://") === 0) {
                        const image: Image = this.generateImageSliderFromResource(slide.file);
                        gridLayout.addChild(image);
                    }

                    // if (slide.title) {
                    //     const title: Label = this.generateTitleSlider(slide.title);
    ...
    private initControls() {
        if (this.totalItems > 1) {
            ...
            // Right arrow label
            const rLabel = new Label();
            rLabel.text = String.fromCharCode(parseInt(arrowType.r, 16));

            // Left arrow layout
            const rStackLayout = new StackLayout();
    ...
    /**
     * Init caroussel autoplay  初始化制造、播放
     */
    ...
    private animateSlides() {
        for (let i = 0; i < this.carouselSlides.getChildrenCount(); i++) {
            ...
            // Get view
            const view: View = this.carouselSlides.getChildAt(i);

            // Get element width + image visibility
            const elementWidth = this.elem.nativeElement.getActualSize().width;
    ...
    /**
     * Load images from URL  从URL加载图片
     * @param src
     * @returns {Image}
     */
    private generateImageSliderFromUrl(src: string): Image {
     ...
     * @returns {Image}
     */
    private generateImageSliderFromFile(path: string): Image {
        const image: Image = new Image();
     ...
     * @returns {Image}
     */
    private generateImageSliderFromResource(path: string): Image {
        const image: Image = new Image();
        const pathRaw: string = path.replace("res://", "");
     ...
     * @returns {Label}
     */
    private generateTitleSlider(title: string): Label {
